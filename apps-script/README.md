...
3. 함수 선택 메뉴에서 `setupAuthSheets`를 실행하고 권한을 승인합니다.
4. `setAuthPepper`를 한 번 실행합니다.
5. 시트에 `Users`, `Sessions` 탭이 생성됐는지 확인합니다.
6. `배포 → 새 배포 → 웹 앱`을 선택합니다.
7. 실행 사용자는 `나`, 액세스 권한은 블로그 공개 범위에 맞게 설정합니다.
8. 생성된 `/exec` URL을 블로그의 API URL로 사용합니다.

`setAuthPepper`를 다시 실행하면 기존 비밀번호 및 세션 검증이 모두 실패합니다. 운영 중에는 다시 실행하지 마세요.
